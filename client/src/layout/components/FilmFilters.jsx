...
const FilmFilters = ({ onSearch }) => {
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        ...
        console.error('Error fetching filters:', error);
        // Статические значения на случай ошибки
        setUniqueCountries(['США', 'Великобритания', 'Франция', 'Германия', 'Япония']);
        setUniqueGenres(['Боевик', 'Драма', 'Комедия', 'Ужасы', 'Фантастика']);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSearch = () => {
    onSearch(filters); // Передаем текущие фильтры
  };

  if (loading) {
    return <div className="mb-4">Загрузка фильтров...</div>;
  }
